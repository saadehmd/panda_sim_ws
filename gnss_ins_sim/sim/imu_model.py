...
gps_mid_accuracy = {'stdp': np.array([1.5, 1.5, 2.5]),
                    'stdv': np.array([0.02, 0.02, 0.02])}
                    
gps_high_accuracy = {'stdp': np.array([1.0, 1.0, 1.8]),
...
class IMU(object):
    ...
    def __init__(self, accuracy='low-accuracy', axis=6,\
        ...
        if gps:
            self.gps = True
            if gps_opt is None:
                self.gps_err = gps_high_accuracy
                
            elif isinstance(gps_opt, str):
                if gps_opt == 'low-accuracy':
                    self.gps_err = gps_low_accuracy
                elif gps_opt == 'mid-accuracy':
                    self.gps_err = gps_mid_accuracy
                elif gps_opt == 'high-accuracy':
                    self.gps_err = gps_high_accuracy
                else:
                    raise ValueError('gps_err is not a valid string.')
